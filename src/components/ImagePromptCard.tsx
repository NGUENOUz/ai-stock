...
const ImagePromptCard: React.FC<ImagePromptCardProps> = ({
  ...
  return (
    <div
      className={`group relative cursor-pointer rounded-3xl overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.18)] flex flex-col h-full bg-white/15 dark:bg-neutral-900/25 border border-white/20 dark:border-white/10 backdrop-blur-xl transition-all duration-500 hover:shadow-[0_0_26px_rgba(139,92,246,0.25)] ${
    ...
    >
      {(isLocked || isQuotaReached) && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/70 backdrop-blur-sm p-4 text-center rounded-3xl">
          ...
          <p className="font-bold text-lg text-white drop-shadow-[0_0_8px_rgba(139,92,246,0.3)]">
            {isLocked ? "ACCÈS PREMIUM" : "QUOTA ÉPUISÉ"}
          </p>
          <Link href="/pricing" className="text-purple-400 text-sm mt-1 hover:underline">
      ...
      <div className="p-4 flex flex-col justify-between flex-grow">
        <div>
          ...
          <div className="text-xs text-purple-400 mb-3">{prompt.style}</div>
          <p
          ...
          <button
            ...
            className={`text-sm flex items-center px-3 py-1.5 rounded-xl font-semibold transition-all duration-300 ${
              isLocked || isQuotaReached
                ...
                : "bg-gradient-to-br from-purple-500 to-blue-600 text-white hover:scale-[1.05] shadow-[0_3px_10px_rgba(139,92,246,0.3)]"
            }`}
