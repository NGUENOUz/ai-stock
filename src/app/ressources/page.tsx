...
export default function ResourcesPage() {
  ...
  return (
      ...
      <div className="sticky top-16 lg:top-0 z-40 bg-[#F8FAFC]/90 dark:bg-[#0B1120]/90 backdrop-blur-xl border-y border-slate-200/50 dark:border-slate-800/50 py-3 shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
           ...
           <MobileFilterBar
             ...
             onFilterOpen={() => setIsMobileFiltersOpen(true)}
             activeFiltersCount={(activeTab !== "Tous" ? 1 : 0) + (activeSort !== "Populaires" ? 1 : 0)}
           />
    ...
    resultCount={mockResources.length}
    onReset={() => { setActiveTab("Tous"); setActiveSort("Populaires"); } }
    groups={[
      {
        id: "type",
        ...
        options: ["Tous", "Guides", "Templates", "Ebooks", "Cheatsheets", "Outils"].map((t) => ({ value: t, label: t })),
      },
      {
        id: "sort",
        label: "Trier par",
        ...
        options: ["Populaires", "Récents", "Mieux notés"].map((s) => ({ value: s, label: s })),
      },
      {
        id: "price",
        label: "Prix",
        ...
        value: "Tous",
        onChange: () => { },
        options: ["Tous", "Gratuit", "Payant"].map((p) => ({ value: p, label: p })),
      },
              ...
              <div className="space-y-1">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm transition-colors ${activeCategory === cat.name
                  ...
                  >
                    <span>{cat.name}</span>
                    <span className={`text-[11px] px-2 py-0.5 rounded-full font-bold ${activeCategory === cat.name ? "bg-primary/20" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}>{cat.count}</span>
                  </button>
                ))}
              </div>
            </div>
