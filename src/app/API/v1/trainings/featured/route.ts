...
export async function GET(request: NextRequest) {
  try {
    ...
    const trainings = await trainingService.getFeaturedTrainings(limit);

    return NextResponse.json(createSuccessResponse(trainings));
  } catch (error: any) {
    console.error('Error fetching featured trainings:', error);
    return NextResponse.json(
      createErrorResponse('FETCH_ERROR', error.message || 'Erreur lors de la récupération des formations en vedette'),
      { status: 500 }
    );
  }
}
